...
impl Vector {
    ...
    pub fn magnitude(&self) -> f32 { self.dot_product(self).sqrt() }
    pub fn normalize(&self) -> Vector { *self / self.magnitude() }
    pub fn dot_product(&self, other: &Vector) -> f32 { (self.0 * other.0) + (self.1 * other.1) }
    pub fn orthogonal(&self) -> Vector { Vector(self.1, -self.0) }
